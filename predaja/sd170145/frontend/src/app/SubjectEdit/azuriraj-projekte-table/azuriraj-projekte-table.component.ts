...
@Component({
  selector: 'app-azuriraj-projekte-table',
  templateUrl: './azuriraj-projekte-table.component.html',
  styleUrls: ['./azuriraj-projekte-table.component.css']
})
export class AzurirajProjekteTableComponent implements OnInit {
  ...
  pocetak() {
    this.subjectService.getGeneralSubjectInfoByObjectId(this.activeRouted.snapshot.params.id).subscribe((result: any) => {
      ...
      this.urlToApply = `${environment.api}/predmetiSacuvaniFajlovi/${subjectId}/projekti`;
      this.initForApply();
    })
  }
  ...
  azurirajMaterijale() {
    ...
    this.subjectService.azurirajProjekteMaterials(this.materials, this.activeRouted.snapshot.params.id, this.projectId).subscribe((result: any) => {
      this.message = result;
    }, (err) => {
  ...
  download(filename: string = null): void {
    ...
    this.http.get(`${this.urlToApply}/${filename}`, { responseType: 'blob' as 'json' }).subscribe(
      (response: any) => {
